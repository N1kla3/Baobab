...
std::string ForElement::GetText()
{
    ...
    {
        if (m_Owner.lock()->GetVariableType(m_Name) != m_Children[i++]->GetType())
            throw "Iteration variable have not appropriate type ";
    }
    else
...
void ForElement::SetName(const std::string& name) { m_Name = name; }
